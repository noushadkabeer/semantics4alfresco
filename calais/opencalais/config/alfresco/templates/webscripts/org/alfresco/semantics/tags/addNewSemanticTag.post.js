...
function addNewSemanticTag(tagName, semanticCategoryName, nodeRef, normalizedName, latitude, longitude, linkedDataURI, website, stockTicker)
{
   ...
   var catNodes = classification.getRootCategories("cm:semantictaggable");
   ...
   if (semanticCatNode == null)
   {
      ...
      semanticCatNode = classification.createRootCategory("cm:semantictaggable", semanticCategoryName);
   }
   ...
   if (semanticCatNode != null)
   {
      try
      {
         ...
         if (tagNode != null)
         {
            ...
            {
               tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}normalizedName"] = normalizedName;
            }
            if (latitude != null)
            {
               tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}latitude"] = latitude;
            }
            if (longitude != null)
            {
               tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}longitude"] = longitude;
            }
            if (linkedDataURI != null)
            {
               tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}URI"] = linkedDataURI;
            }
            if (website != null)
            {
               tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}webaddress"] = website;
            }
            if (stockTicker != null)
            {
               // attribute currently not defined, not stored yet
               // tagNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}ticker"] = stockTicker;
            }
