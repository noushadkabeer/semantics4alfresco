 ...
 *
 *
 * Alfresco webscript from tomcat\webapps\alfresco\WEB-INF\classes\alfresco\templates\webscripts\org\alfresco\slingshot\search
 * modified to handle stanbol integration semantic tag/category search
 * 
 ...
 *       term: search terms
 *       tag: search tag
 *       semanticTag: search semantic tag
 *       semanticCategory: search semantic category
...
function getSearchResults(params)
{
   var nodes,
      ...
      semanticCategory = params.semanticCategory,
   ...
   else if (semanticTag !== null && semanticTag.length !== 0)
   {
      var escapedTagName = search.ISO9075Encode(semanticTag);
      if (semanticCategory !== null && semanticCategory.length !== 0)
      {
         var escapedSemanticCat = search.ISO9075Encode(semanticCategory);
         ftsQuery = 'PATH:\"/cm:semantictaggable2/cm:' + escapedSemanticCat + '/cm:' + escapedTagName + '//*\"';                                          
      }   
      else
      {
         ftsQuery = 'PATH:\"/cm:semantictaggable2//cm:' + escapedTagName + '//*\"';  
      }
   }   
