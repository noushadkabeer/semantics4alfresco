...
function semanticTagQuery(nodeRef, maxResults, semanticCategory, shareSiteId)
{
   ...
   {
      query += "\" AND PATH:\"/cm:semantictaggable/cm:" + semanticCategory + "//*\"";
   }
   else
   {
      query += "\" AND PATH:\"/cm:semantictaggable//*\"";
   }   
   ...
   {   
      ...
      {
         for each(tag in taggedNode.properties["{http://www.alfresco.org/model/content/semantic/1.0}taggable"])
         {
            if (tag != null  && (semanticCategory == null || semanticCategory == tag.parent.name ))
            {
				...
				var latitude = tag.properties["{http://www.alfresco.org/model/content/semantic/1.0}latitude"];
				var longitude = tag.properties["{http://www.alfresco.org/model/content/semantic/1.0}longitude"];
